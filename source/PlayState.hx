...
class PlayState extends MusicBeatState
{
	...
	var spintime:Array<Int>;
	var maniachangetest:Array<Int>;
	var maniarecup:Array<Int>;
	var spin2:Array<Int>;
	...
	private var botPlayFalse:FlxText;
	// Replay shit
	...
	override public function create()
	{
				...
				case 'train':
					{
							defaultCamZoom = 0.9;
							curStage = 'train';
							var bg:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('bg/pared', 'akali'));
							bg.antialiasing = FlxG.save.data.antialiasing;
							bg.scrollFactor.set(0.9, 0.9);
							bg.active = false;
							add(bg);
		
							var stageFront:FlxSprite = new FlxSprite(-650, -300).loadGraphic(Paths.image('bg/suelo', 'akali'));
							stageFront.setGraphicSize(Std.int(stageFront.width * 1.1));
							stageFront.updateHitbox();
							stageFront.antialiasing = FlxG.save.data.antialiasing;
							stageFront.scrollFactor.set(0.9, 0.9);
		...
		switch (curStage)
		{
			...
			case 'train':
				gf.x -= 130;
				camPos.set(boyfriend.getGraphicMidpoint().x - 490);
				bfxoffset -= 130;
			case 'schoolEvil':
				bfxoffset += 200;
				bfyoffset += 220;
		...
		if (!PlayStateChangeables.flip)
			...
			{
				curcol2 = col[characterCol.indexOf(dad.curCharacter)]; // Dad Icon
				curcol = col[characterCol.indexOf(boyfriend.curCharacter)]; // Bf Icon
				...
				'health', 0, 2);
				healthBar.scrollFactor.set();
				healthBar.createFilledBar(curcol2, curcol);
			}
		// healthBar
		add(healthBar);
		...
		overhealthBar.createFilledBar(curcol, curcol2);
		// healthBar
		add(overhealthBar);
		...
		botPlayState = new FlxText(healthBarBG.x + healthBarBG.width / 2 - 75, healthBarBG.y + (PlayStateChangeables.useDownscroll ? 100 : -100), 0, "BOTPLAY DEV VERSION", 20);
		botPlayState.setFormat(Paths.font("vcr.ttf"), 42, FlxColor.WHITE, RIGHT, FlxTextBorderStyle.OUTLINE,FlxColor.BLACK);
		botPlayState.scrollFactor.set();
		botPlayState.borderSize = 4;
		botPlayState.borderQuality = 2;

		botPlayFalse = new FlxText(healthBarBG.x + healthBarBG.width / 2 - 75, healthBarBG.y + (PlayStateChangeables.useDownscroll ? 100 : -100), 0, "Press O or B to Desactive Botplay", 20);
		botPlayFalse.setFormat(Paths.font("vcr.ttf"), 42, FlxColor.WHITE, RIGHT, FlxTextBorderStyle.OUTLINE,FlxColor.BLACK);
		botPlayFalse.scrollFactor.set();
		botPlayFalse.borderSize = 4;
		botPlayFalse.borderQuality = 2;
		botPlayFalse.y = 100;
		botPlayFalse.screenCenter();

		if(PlayStateChangeables.botPlay && !loadRep) add(botPlayFalse);
	...
	override public function update(elapsed:Float)
	{
		...
		if(FlxG.keys.justPressed.B && curSong == 'Sonic')
			{
              nieve--;
			}
			else 
	...
	*/
	...
	override function stepHit()
	{
		...
		if(curStep == 2098 && curSong == 'Sukrol')
			{
				dad.playAnim('letsgo', true);
			}

	/*	var lightsDownTrue = [2098];
		var lightsDownFalse = [2123];
		if(curSong == "Sukrol" && curStep == 1) 
		{
		...
		if(curSong == "Sukrol")
		{
			...
			{
				if (curStep == lightsDownTrue[i])
					godemo(true);
			}
			for (i in 0...lightsDownFalse.length)
			{
				if (curStep == lightsDownFalse[i])
					godemo(false);
			}
	...
	override function beatHit()
	{
		...
		/*var lightsDownTrue = [31];
		var lightsDownFalse = [127];
		if(curSong == "Sukrol" && curBeat == 1) 
		{
		...
		if(curSong == "Sukrol" && curBeat == 524) 
			{
				dad.playAnim('letsgo', true);
			}

		var lightsDownTrue = [223];
		var lightsDownFalse = [260];
		if(curSong == "Sukrol" && curBeat == 1) 
		{
		...
		var lightsDownTrue = [283];
		var lightsDownFalse = [303];
		if(curSong == "Sukrol" && curBeat == 1) 
		{
		...
		var lightsDownTrue = [526];
		var lightsDownFalse = [530];
		if(curSong == "Sukrol" && curBeat == 1) 
		{
		...
		/*var lightsDownTrue = [351];
		var lightsDownFalse = [366];
		if(curSong == "Sukrol" && curBeat == 1) 
		{
		...
		var lightsDownTrue = [382];
		var lightsDownFalse = [431];
		if(curSong == "Sukrol" && curBeat == 1) 
		{
		...
		var lightsDownTrue = [463];
		var lightsDownFalse = [495];
		if(curSong == "Sukrol" && curBeat == 1) 
		{
		...
		var lightsDownTrue = [495];
		var lightsDownFalse = [542];
		if(curSong == "Sukrol" && curBeat == 1) 
		{
		...
		if(curSong == "Sukrol")
		{
			...
			{
				if (curBeat == lightsDownTrue[i])
					camera_acercamiento(true);
			}
			for (i in 0...lightsDownFalse.length)
			{
				if (curBeat == lightsDownFalse[i])
					camera_acercamiento(false);
			}
		...
		if (boyfriend.curCharacter == 'bf' && curSong == 'Tutorial')
			{
             switch (curBeat)
			 {
			 case 10:
			 ...
			 if (spintime.contains(curBeat))
				{
					strumLineNotes.forEach(function(tospin:FlxSprite)  //Spin 1
					{
			...
			if (curBeat == 31 && curSong == 'Sukrol' && curSong == 'sukrol' && SONG.song.toLowerCase() == 'sukrol' &&  SONG.song.toLowerCase() == 'Sukrol')
				{
					SONG.speed = 3.3;
				}
			
			if (curBeat == 127 && curSong == 'Sukrol' && curSong == 'sukrol' && SONG.song.toLowerCase() == 'sukrol' &&  SONG.song.toLowerCase() == 'Sukrol')
				{
					SONG.speed = 2.8; 
				}
			if (curBeat == 223 && curSong == 'Sukrol' && curSong == 'sukrol' && SONG.song.toLowerCase() == 'sukrol' &&  SONG.song.toLowerCase() == 'Sukrol')
				{
					SONG.speed = 3.5;
				} 
			if (curBeat == 283 && curSong == 'Sukrol' && curSong == 'sukrol' && SONG.song.toLowerCase() == 'sukrol' &&  SONG.song.toLowerCase() == 'Sukrol')
				{
				SONG.speed = 3.6;
				}
			if (curBeat == 303 && curSong == 'Sukrol' && curSong == 'sukrol' && SONG.song.toLowerCase() == 'sukrol' &&  SONG.song.toLowerCase() == 'Sukrol')
				{
				SONG.speed = 3.7;
				}
			if (curBeat == 351 && curSong == 'Sukrol' && curSong == 'sukrol' && SONG.song.toLowerCase() == 'sukrol' &&  SONG.song.toLowerCase() == 'Sukrol')
					{
				SONG.speed = 3.8;
			}
			if (curBeat == 366 && curSong == 'Sukrol' && curSong == 'sukrol' && SONG.song.toLowerCase() == 'sukrol' &&  SONG.song.toLowerCase() == 'Sukrol')
					{
				SONG.speed = 3.9;
				}
			if (curBeat == 382 && curSong == 'Sukrol' && curSong == 'sukrol' && SONG.song.toLowerCase() == 'sukrol' &&  SONG.song.toLowerCase() == 'Sukrol')
				{
				SONG.speed = 4.0;
			}
			if (curBeat == 431 && curSong == 'Sukrol' && curSong == 'sukrol' && SONG.song.toLowerCase() == 'sukrol' &&  SONG.song.toLowerCase() == 'Sukrol')
		{
			SONG.speed = 4.1;
				}
			if (curBeat == 463 && curSong == 'Sukrol' && curSong == 'sukrol' && SONG.song.toLowerCase() == 'sukrol' &&  SONG.song.toLowerCase() == 'Sukrol')
				{
            	SONG.speed = 4.3;
				}
			if (curBeat == 495 && curSong == 'Sukrol' && curSong == 'sukrol' && SONG.song.toLowerCase() == 'sukrol' &&  SONG.song.toLowerCase() == 'Sukrol')
				{
				SONG.speed = 4.5;
				}
			if (curBeat == 525 && curSong == 'Sukrol' && curSong == 'sukrol' && SONG.song.toLowerCase() == 'sukrol' &&  SONG.song.toLowerCase() == 'Sukrol')
				{
					camHUD.alpha = 0;
					camNotes.alpha = 0;
				}

				if (curSong == 'Sukrol' && SONG.song.toLowerCase() == 'sukrol' &&  SONG.song.toLowerCase() == 'Sukrol')
					...
					if (boyfriend.curCharacter == 'bf' && curSong == 'Tutorial') //song events
						{
							switch(curBeat) //guide for anyone looking at this, switching mid song needs to be mania + 10
							{
								...
								case 57: //switched it to modcharts! (can still be hardcoded though)
									//2 key
									//switchMania(17);
								case 125: 
									//4 key
											...
											new FlxTimer().start(0.01, function(swagTimer:FlxTimer)
												{
													...
													if (camHUD.alpha <= 0 /*&& camNotes.alpha <= 0*/)
													{
														trace("done");
														new FlxTimer().start(40, function(swagTimer:FlxTimer)
														{
															new FlxTimer().start(0.01, function(swagTimer:FlxTimer)
															{
																camHUD.alpha += 0.01;
																/*camNotes.alpha += 0.01;*/
																if (camHUD.alpha != 1 /*&& camNotes.alpha != 1*/)
																{
																	swagTimer.reset();
																}
															});
...
function camera_acercamiento(id:Bool) {
	...
	if(id)
		{
			...
			{
				defaultCamZoom = 1.4;  //Zoom que quieres
				camHUD.alpha = 0;
				camNotes.alpha = 0;
			});
			...
			new FlxTimer().start(15 , function(tmr:FlxTimer)
			{
				defaultCamZoom = 0.9;  //Zoom de tu escenario
				camHUD.alpha = 1;
				camNotes.alpha = 1;
			});
...
function neomode(id:Bool){ //ahuevo
 if(id)
	{
      camHUD.alpha = 0;
	  camNotes.alpha = 0;
